...
final class ResamplingSuite extends munit.FunSuite:
  ...
  test("named bootstrap policies exactly expand to the explicit policy route") {
    ...
    val population = right(Summary.of("resample4s/size", space.size.toLong))
  ...
  test("Fail and Redraw resolve empty OOB during compilation") {
    ...
    def firstSeedWhere(
        design: Bootstrap,
        matches: Either[DesignError, Compiled[Split[Draw], Coverage]] => Boolean
    ): Long =
      Vector
        .range(0, 1000)
        .find { value =>
          matches(design.compile(space, Seed.fromLong(value.toLong)))
        }
        .getOrElse(fail("expected a matching deterministic bootstrap seed"))
        .toLong

    val failing: Either[
      DesignError,
      Compiled[Split[Draw], Coverage]
    ] =
    ...
    def isExpectedFailure(
      ...
      result match
        case Left(error) =>
          error == DesignError.EmptyOutOfBag(UnitKey(0, 0), 1)
        case Right(_) => false

    val failDesign = Bootstrap(1, OobPolicy.Fail)
    val failSeed = firstSeedWhere(failDesign, isExpectedFailure)
    assertEquals(
      failDesign.compile(space, Seed.fromLong(failSeed)),
      failing
    )

    val redrawOnce = Bootstrap(1, OobPolicy.Redraw(1))
    val redrawSeed = firstSeedWhere(redrawOnce, isExpectedFailure)
    assertEquals(
      redrawOnce.compile(space, Seed.fromLong(redrawSeed)),
      failing
    )

    val redrawMany = Bootstrap(1, OobPolicy.Redraw(32))
    val acceptedSeed = firstSeedWhere(redrawMany, _.isRight)
    val accepted =
      right(
        redrawMany.compile(space, Seed.fromLong(acceptedSeed))
      )
    assert(
