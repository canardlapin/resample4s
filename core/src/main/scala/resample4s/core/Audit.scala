...
final class DigestValue private (private val bytes: IArray[Byte]):
  ...
  private[resample4s] def unsafeAt(index: Int): Byte = bytes(index)
...
object DigestValue:
  ...
  private[resample4s] def fromOwned(bytes: IArray[Byte]): DigestValue =
...
private[resample4s] object FingerprintEquality:
