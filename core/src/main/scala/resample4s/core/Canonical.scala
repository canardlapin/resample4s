...
private[resample4s] object Utf8:
...
private[resample4s] object Identifiers:
...
final class CanonicalWriter private[resample4s] (
  ...
  private[resample4s] def beginSequenceUnchecked(length: Int): Unit =
    beginSequence(length) match
      case Right(_) => ()
      case Left(error) =>
        throw new IllegalStateException(s"invalid owned sequence: $error")

  private[resample4s] def variantUnchecked(tag: String): Unit =
    variant(tag) match
      case Right(_) => ()
      case Left(error) =>
        throw new IllegalStateException(s"invalid owned variant: $error")

  private[resample4s] def error: Option[DigestError] = failure

  private[resample4s] def rawByte(value: Int): Unit =
    emit(IArray.unsafeFromArray(Array(value.toByte)))

  private[resample4s] def rawInt(value: Int): Unit =
    emit(
  ...
  private[resample4s] def rawLong(value: Long): Unit =
    emit(
      IArray.unsafeFromArray(
        Array(
          (value >>> 56).toByte,
...
private[resample4s] object CanonicalWriter:
...
object DescriptorValue:
  private final class IntValue(value: Int) extends DescriptorValue:
    private[resample4s] def write(out: CanonicalWriter): Unit = out.int(value)

  private final class LongValue(value: Long) extends DescriptorValue:
    private[resample4s] def write(out: CanonicalWriter): Unit = out.long(value)

  private final class BoolValue(value: Boolean) extends DescriptorValue:
    private[resample4s] def write(out: CanonicalWriter): Unit = out.bool(value)
  ...
  private[resample4s] def variantUnchecked(
      tag: String,
      value: DescriptorValue
  ): DescriptorValue =
...
private[resample4s] object CanonicalDesign:
  def designChunks(
  ...
  private def writeDesign(
    ...
    out.textUnchecked("resample4s/design/v1")
  ...
  private def writeLabelSet(
    ...
    labels match
      case Vector(single) =>
        out.textUnchecked("resample4s/labels/v1")
        writeLabels(single, out)
      case multiple =>
        out.textUnchecked("resample4s/label-set/v1")
        out.beginSequenceUnchecked(multiple.length)
        multiple.foreach(writeLabels(_, out))
