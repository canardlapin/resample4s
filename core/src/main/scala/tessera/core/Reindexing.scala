...
/** A total function from one finite ordinal into another.
  ...
  * inside Tessera after dimensions have been validated.
  */
sealed trait Reindexing:
  ...
  private[tessera] def unsafeAt(index: Int): Int

sealed trait Injective extends Reindexing

private[tessera] object ReindexingValidation:
  def validateRange(
      values: IArray[Int],
      codomain: Int
...
private[tessera] sealed trait SelectionBacking:
  def size: Int
  def codomain: Int
  def unsafeAt(index: Int): Int
  def materialize: IArray[Int]

private[tessera] object SelectionBacking:
  ...
  final class Block(
      private[tessera] val partition: FoldPartition,
      private[tessera] val blockId: Int
  ) extends SelectionBacking:
  ...
  final class ComplementOf(
      private[tessera] val base: Selection
  ) extends SelectionBacking:
...
/** A strictly increasing, injective finite reindexing.
  ...
  */
final class Selection private[tessera] (
    private[tessera] val backing: SelectionBacking
) extends Injective:
  def domain: Int = backing.size
  def codomain: Int = backing.codomain
  private[tessera] def unsafeAt(index: Int): Int = backing.unsafeAt(index)
...
object Selection:
  ...
  private[tessera] def fromSortedOwned(
      values: Vector[Int],
      codomain: Int
  ): Selection =
    new Selection(
...
final class Draw private (
  ...
  private[tessera] def unsafeAt(index: Int): Int = values(index)
...
object Permutation:
  ...
  private[tessera] def fromOwned(values: IArray[Int]): Permutation =
    new Permutation(values)

  given CanEqual[Permutation, Permutation] = CanEqual.derived

private[tessera] object ReindexingEquality:
