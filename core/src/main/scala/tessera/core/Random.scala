...
object DesignKey:
  private[tessera] def fromLong(value: Long): DesignKey = value
...
final case class StreamSegment private[tessera] (
    domain: StreamDomain,
    ordinal: Int
) derives CanEqual
...
final class StreamPath private (
  ...
  private[tessera] def unsafeAt(index: Int): StreamSegment = segments(index)

  private[tessera] def appendUnchecked(
      domain: StreamDomain,
      ordinal: Int
  ): StreamPath =
...
final class Rand private (private val state: Long):
  ...
  private[tessera] def nextBigIntBoundedUnsafe(
      upperExclusive: BigInt
  ): (Rand, BigInt) =
...
object Rand:
  ...
  private[tessera] def mix64(value: Long): Long =
    var mixed = value
    mixed = (mixed ^ (mixed >>> 30)) * Mix1
    mixed = (mixed ^ (mixed >>> 27)) * Mix2
    mixed ^ (mixed >>> 31)

  private[tessera] def unsigned(value: Long): BigInt =
