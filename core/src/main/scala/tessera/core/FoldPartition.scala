...
final class FoldPartition private (
  ...
  private[tessera] def assignmentUnsafe(index: Int): Int =
    assignments match
      case Some(values) => values(index)
      case None         => index

  private[tessera] def blockSizeUnsafe(fold: Int): Int =
    packedMembers match
      case Some(values) => values(fold).length
      case None         => 1

  private[tessera] def blockMemberUnsafe(fold: Int, index: Int): Int =
    packedMembers match
      case Some(values) => values(fold)(index)
      case None         => fold

  private[tessera] def blockMembersUnsafe(fold: Int): IArray[Int] =
    packedMembers match
      case Some(values) => values(fold)
      case None         => OwnedArrays.ints(fold)

  private[tessera] def residentElementsUpperBound: Long =
    assignments match
...
object FoldPartition:
  ...
  private[tessera] def singletonIdentity(n: Int): FoldPartition =
...
object Split:
  ...
  private[tessera] def unsafe[A <: Reindexing](
      analysis: A,
      assessment: Selection
  ): Split[A] =
