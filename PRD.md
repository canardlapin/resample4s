...
**Coverage is a type-level capability, not a runtime flag.** One-repeat partitioning designs (KFold and its grouped/stratified variants, LOO, LOGO, and jackknife delete-1) compile to `Plan[Split[Selection], Coverage.ExactOnce]`; `ExactOnce <: Exact`, where `Exact` means that assessments partition the population within each repeat. Calling `.repeat(r)` preserves `Exact` but deliberately drops `ExactOnce`: a two-repeat K-fold assesses every row twice over the whole plan and cannot drive one out-of-fold reconstruction. Holdout, Monte Carlo, Bootstrap, and delete-*d* for `d > 1` compile to `Plan[Split[…], Coverage]`. Public `map` and `zip` forget coverage: they return `Plan[…, Coverage]` because an arbitrary payload transform does not preserve partition witnesses. Library-internal `mapPreservingCoverage` retains `Cov` only for transforms the implementation has proven safe (NestedCrossValidation embedding). This is what lets Alder's `CompleteResampler` (Alder D19) accept `ExactOnce` through a total factory while rejecting Holdout, Bootstrap, repeated K-fold, and publicly mapped plans statically.
...
object DesignDescriptor:
  def named(
    algorithm: String,
    fields: (String, DescriptorValue)*): Either[DesignError, DesignDescriptor]
  def of(
...
object GeneralPlanSpec:
  def apply[A](
    ...
    cost: PlanCost)(
    unit: UnitKey => A,
    encoder: CanonicalAssignmentEncoder[A]): GeneralPlanSpec[A]
...
object Labels:
  def of(codes: IArray[Int], cardinality: Int): Either[DesignError, Labels]
  ...
  // Labels.dense(codes) accepts sparse/arbitrary input codes and infers n;
...
**Grouped bootstrap** (`Bootstrap.grouped(times, groups, policy)`):
...
| `Bootstrap(t, policy)` | `t` | **O(t)** child seeds | O(n) regeneration |
| `Bootstrap.grouped(t, groups, policy)` | `t` | O(n + t) labels + child seeds | O(g + L) lazy split; +O(n) if OOB materialized |
...
**Empty out-of-bag.** For small `n`, a bootstrap draw has positive probability of covering the whole population; each particular row's omission probability is `(1 − 1/n)^n → e⁻¹`, so neither OOB behavior nor the full-coverage event may be waved away with an asymptotic slogan. Policy is always explicit, either through the general constructor or a named preset:
```scala
enum OobPolicy:
  case Allow                      // empty assessment permitted; the Split ctor admits it for Draw analyses
  case Redraw(maxAttempts: Int)   // deterministic seed advance per attempt
  case Fail

Bootstrap.unconditional(times)
Bootstrap.redrawing(times, maxAttempts = 8)
Bootstrap.failOnEmptyOob(times)
Bootstrap(times, policy)          // general explicit route
```
`GroupedBootstrap` exposes the same three named presets; `Bootstrap.grouped(times, groups, policy)` is its general explicit route. No bare bootstrap constructor chooses a distributional policy silently.
...
- `Bootstrap(times, policy)` and its named presets have shape `(times, 1)`. Each unit makes exactly `n` independent uniform draws from `[0, n)` using bounded rejection sampling. Their generation order is the analysis `Draw`; assessment is the ascending complement of its support.
...
| **D2** | Coverage is a **type parameter** on `Plan[+A, +Cov <: Coverage]`: `Exact` means once per repeat and `ExactOnce <: Exact` additionally proves one repeat. Public `map`/`zip` drop to `Coverage`; `.repeat` preserves `Exact` but drops `ExactOnce`; package-private `mapPreservingCoverage` retains `Cov` only for proven transforms. | Alder D19 requires exactly once over the entire plan to determine which programs are legal. Holdout lacks even per-repeat coverage, while repeated K-fold has per-repeat coverage but would produce multiple OOF values per row. Arbitrary `map` must not forge those capabilities. |
